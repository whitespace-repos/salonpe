            ...
            plugin.each(function (i, wrapper) {
                ...
                if (plugin.settings.preloaded.length) {
                    ...
                    for (let i = 0; i < plugin.settings.preloaded.length; i++) {
                        $uploadedContainer.append(createImg(plugin.settings.preloaded[i].src, plugin.settings.preloaded[i].id, true , plugin.settings.preloaded[i].status));
                    }
...
//            });
            ...
            //$radio.on('change', fileSelectHandler.bind($container));
        ...
        let createImg = function (src, id, preloaded , status) {
            ...
            let $container = $('<div>', {class: 'uploaded-image'}),
                ...
                // Create the delete button
                //$check = $('<input>', {class: 'main-image' , type : 'radio' , name : 'main_image' }).appendTo($container),
...
//
//                $(".image-uploader .uploaded .uploaded-image .main-image").removeClass("checked"); 
...
//            });
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {  
                $(".alert").html(`The file "${file.name}" does not match with the accepted file extensions: "${plugin.settings.extensions.join('", "')}"`).show();              
        ...
        let validateMaxSize = function (file) {
            ...
            if (file.size > plugin.settings.maxSize) {
                $(".alert").html(`The file "${file.name}" exceeds the maximum size of ${plugin.settings.maxSize / 1024 / 1024}Mb`).show();              
                setTimeout(function(){  $(".alert").hide(); },3000);
                return false;
            }
        ...
        let validateMaxFiles = function (index, file) {
            ...
            if ((index + dataTransfer.items.length + plugin.settings.preloaded.length) >= plugin.settings.maxFiles) {
                $(".alert").html(`The file "${file.name}" could not be added because the limit of ${plugin.settings.maxFiles} files has been reached`).show();              
                setTimeout(function(){  $(".alert").hide(); },3000);
                return false;
            }
